...
int main(int argc, char *argv[]) {
    if(argc != 5) {
        cout << "Usage: " << argv[0] << " <encryptImg> <decryptImg> <BlockWidth> <encryptKey>" << endl;
        exit(EXIT_FAILURE);
    }
    ...
    allocation_tableau(imgOut, OCTET, 1000*1000);

    // initialisation des paramettres des blocs de l'img dechiffree
    int nbBlockWidth = atoi(argv[3]);
    int nbBlock = nbBlockWidth*nbBlockWidth;

    int oldWidthBlock = 1000/nbBlockWidth;
    int oldSizeBlock = oldWidthBlock*oldWidthBlock;

    // initialisation des paramettres des blocs de l'img chiffre
    int newWidthBlock = width/nbBlockWidth;
    int newSizeBlock = newWidthBlock*newWidthBlock;
    ...
    for(int i = 0; i < nbBlock; i++) {
        ...
        if(i == 0) {
            ...
            oldIndexFirst = (i - (i % nbBlockWidth)) * oldSizeBlock + ((i % nbBlockWidth) * oldWidthBlock);
        }
        if(newBlockIndex == 0) {
            newIndexFirst = 0;
        } else {
            newIndexFirst = (newBlockIndex - (newBlockIndex % nbBlockWidth)) * newSizeBlock + ((newBlockIndex % nbBlockWidth) * newWidthBlock);
        }


        newIndex = newIndexFirst + (newWidthBlock / 2) + ((newWidthBlock / 2) * width);

        // boucle pour chaque pixel du bloc
        for(int x = 0; x < newWidthBlock; x++) {
            for(int y = 0; y < newWidthBlock; y++) {
                // calcul du nouvel indice du pixel dans le bloc
                // newIndex = newIndexFirst + x + (y * width);
                oldIndex = oldIndexFirst + x + (y * width);

                // melange
                imgOut[oldIndex] = imgIn[newIndex];
            }
        }
